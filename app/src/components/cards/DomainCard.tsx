...
const statusConfig = {
  'For Sale': { color: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20', icon: Tag },
  Premium: { color: 'bg-amber-500/10 text-amber-400 border-amber-500/20', icon: Sparkles },
  Sold: { color: 'bg-zinc-500/10 text-zinc-400 border-zinc-500/20', icon: Shield },
  Reserved: { color: 'bg-blue-500/10 text-blue-400 border-blue-500/20', icon: Shield },
};
...
export function DomainCard({ domain, status, category, price, description, buyUrl, marketplace = 'GoDaddy Marketplace', index = 0 }: DomainCardProps) {
  const { t } = useLanguage();
  const d = t.domains;
    ...
    >
      ...
      <div className="relative h-full bg-zinc-900/50 backdrop-blur-sm border border-white/5 rounded-2xl p-6 hover:border-emerald-500/30 transition-all duration-500">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <Badge variant="outline" className={`text-xs font-medium ${categoryColors[category] || 'bg-zinc-500/10 text-zinc-400 border-zinc-500/20'}`}>
            {category}
          </Badge>
        </div>

        {/* Domain Name */}
        <div className="mb-4 relative p-3 -m-3 rounded-xl transition-colors hover:bg-white/5">
          ...
          {price && <p className="mt-2 text-lg font-semibold text-emerald-400">{price}</p>}
        </div>

        {description && <p className="text-sm text-zinc-400 mb-6 line-clamp-2">{description}</p>}
        ...
        {status !== 'Sold' ? (
          buyUrl ? (
            <a href={buyUrl} target="_blank" rel="noopener noreferrer" className="block w-full">
              <Button className="w-full bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-semibold transition-all duration-300 group/btn">
                <span>{d.buyNow}</span>
            ...
            <Button onClick={handleInquiry}
              ...
              <span>{d.inquireBuy}</span>
              <ExternalLink className="w-4 h-4 ml-2 group-hover/btn:translate-x-0.5 transition-transform" />
            </Button>
          )
        ) : (
          <Button disabled className="w-full bg-zinc-800/50 text-zinc-500 cursor-not-allowed">
            {d.soldLabel}
          </Button>
        )}
      </div>
